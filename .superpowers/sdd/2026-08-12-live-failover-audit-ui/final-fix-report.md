...
3. BeginMaintenance success now returns its new maintenance ID in `Details`
   while preserving `Code: OK` and the existing Message. Smoke validates the
   direct response's status, code, exact instance message, and positive integer
   key, then calls only `/api/end-maintenance/$MAINTENANCE_KEY`. Failed or
   unrelated responses cause no cleanup call.
...
- `bash tests/functional/test-smoke.sh`: 35 passed, 0 failed, 0 skipped;
  begin returned key 1 and cleanup ended that exact key.
- `git diff --check`: pass.
