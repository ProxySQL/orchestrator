...
func TestMkInsertOdkuThree(t *testing.T) {
	...
	s3 := `INSERT  INTO database_instance
        ...
        `
	a3 := `
		i710, 3306, 0, 710, , 5.6.7, 5.6, MySQL, false, false, STATEMENT, FULL, false, false, , 0, , 0, false, false, 0, 0, false, false, false, , , , , , , false, false, , 0, mysql.000007, 10, , 0, , , {0 false}, {0 false}, 0, 0, [], , , , , , 0, false, false, false, false, 0, false, false, 0, 0, false, false, 0, false, , 0, , false, , , [], , 0, mysql,
		i720, 3306, 0, 720, , 5.6.7, 5.6, MySQL, false, false, STATEMENT, FULL, false, false, , 0, , 0, false, false, 0, 0, false, false, false, , , , , , , false, false, , 0, mysql.000007, 20, , 0, , , {0 false}, {0 false}, 0, 0, [], , , , , , 0, false, false, false, false, 0, false, false, 0, 0, false, false, 0, false, , 0, , false, , , [], , 0, mysql,
		i730, 3306, 0, 730, , 5.6.7, 5.6, MySQL, false, false, STATEMENT, FULL, false, false, , 0, , 0, false, false, 0, 0, false, false, false, , , , , , , false, false, , 0, mysql.000007, 30, , 0, , , {0 false}, {0 false}, 0, 0, [], , , , , , 0, false, false, false, false, 0, false, false, 0, 0, false, false, 0, false, , 0, , false, , , [], , 0, mysql,
		`
...
// TestInstanceProviderTypeDefault verifies that NewInstance has empty ProviderType
// and that the write path defaults it to "mysql".
func TestInstanceProviderTypeDefault(t *testing.T) {
	inst := NewInstance()
	test.S(t).ExpectEquals(inst.ProviderType, "")
}

// TestInstanceProviderTypePersistence verifies that provider_type is included in
// the insert/update SQL when set on an instance.
func TestInstanceProviderTypePersistence(t *testing.T) {
	instances := mkTestInstances()
	instances[0].ProviderType = "postgresql"
	sql1, args1, err := mkInsertOdkuForInstances(instances[:1], false, true)
	test.S(t).ExpectNil(err)
	test.S(t).ExpectTrue(strings.Contains(sql1, "provider_type"))
	// The provider_type value should appear in the args
	argsStr := fmtArgs(args1)
	test.S(t).ExpectTrue(strings.Contains(argsStr, "postgresql"))
}

// TestInstanceProviderTypeDefaultsToMySQL verifies that empty ProviderType
// defaults to "mysql" in the write path.
func TestInstanceProviderTypeDefaultsToMySQL(t *testing.T) {
	instances := mkTestInstances()
	// ProviderType is empty (zero value)
	_, args1, err := mkInsertOdkuForInstances(instances[:1], false, true)
	test.S(t).ExpectNil(err)
	argsStr := fmtArgs(args1)
	test.S(t).ExpectTrue(strings.Contains(argsStr, "mysql"))
}
